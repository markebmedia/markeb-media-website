...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Try exact match first (invoice numbers may not follow the INV-MM{bookingRef} pattern)
    ...
    if (!invoiceRecords || invoiceRecords.length === 0) {
      invoiceRecords = await base('Invoices')
        .select({ filterByFormula: `{Invoice Number} = "${ref}"`, maxRecords: 1 })
        .firstPage();
    }
    ...
    const invoiceRecord = { id: invoiceRecords[0].id, fields: invoiceRecords[0].fields };
    const manualHtml = buildManualInvoiceHTML(invoiceRecord, invoiceRecord.fields['Invoice Number'] || invoiceNum);
...
function buildManualInvoiceHTML(invoiceRecord, invoiceNum) {
  ...
  const finalPrice = parseFloat(inv['Amount'] || 0);
  ...
  const billingAddress = inv['Billing Address'] || '';

  let lineItems = [];
  try { lineItems = JSON.parse(inv['Line Items JSON'] || '[]'); } catch (e) { lineItems = []; }
  ...
  <div class="body">
    ...
    <div class="parties">
      ...
      <div>
        <div class="party-label">Billed to</div>
        <div class="party-name">${clientName}</div>
        <div class="party-detail">${clientEmail}${billingAddress ? `<br>${billingAddress}` : ''}</div>
      </div>
    </div>
